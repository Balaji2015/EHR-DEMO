...
function viewSummary(encounterId, humanId) {
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            //Jira cap - 3020
            $(top.window.document).find('#ProcessFrame')[0].contentDocument.body.style.fontWeight = 'bold';
            $(top.window.document).find('#ProcessFrame')[0].contentDocument.body.style.fontFamily = "Helvetica Neue, Helvetica,Arial,sans-serif";
            $(top.window.document).find('#ProcessFrame')[0].contentDocument.body.style.fontSize = '13px';
