...
var count = 0;
//CAP-951
var unhandeledException = ["is not defined", "$(...)?.mask is not a function", " $(...)?.datetimepicker is not a function", "Internet connection is not detected", "a[b] is not a function", "$scope.RefershGrid is not a function", "Unexpected identifier Window", "$(...).find(...)?.modal is not a function", "unrecognized expression: # iframe", "google.visualization.DataTable"];
...
function ScriptErrorLogEntry(sErrorMessage, sErrorLineNo, sErrorColumnNo, sErrorUrl, sErrorStack, isAlert) {
    ...
    var isUnhandeledException = unhandeledException.some(function (exception) {
        return sErrorMessage.indexOf(exception) !== -1;
    });
