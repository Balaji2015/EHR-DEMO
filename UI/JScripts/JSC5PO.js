...
function OpenModal(data) {
    ...
    //BugID:49829
        ...
        //});
        var obj = new Array();
        var ResultFindReferralPhysician = openModal("frmFindReferralPhysician.aspx?IsMenuLevel=true", 256, 930, obj, "RadWindow1");
...
function OpenErrorIndexing(Mode) {
    StartLoadingImage();
    var obj = new Array();
    openModal("frmErrorIndexing.aspx", 610, 1000, obj, "ctl00_ModalWindow");
}
...
window.addEventListener("offline", (event) => {
    $("#divInternetLoader").css("display", "block");
    sessionStorage.setItem('StartLoading', 'false');
    StopLoadFromPatChart();
    offlineTime = formatAMPM(new Date());
});

//CAP-476 - Check whether system in sleep mode or not
document.addEventListener('visibilitychange', function () {
    if (document.hidden) {
        // Page is hidden, system might be in sleep mode
        bSleep = true;
    } else {
...
window.addEventListener("online", (event) => {
    ...
    var ErrorLogData = {
        ErrorMessage: 'Internet connection is not detected between ' + offlineTime + ' and ' + onlineTime,
