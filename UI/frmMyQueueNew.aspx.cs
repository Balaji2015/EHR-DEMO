...
namespace Acurus.Capella.UI
{
    public partial class frmMyQueueNew : System.Web.UI.Page
    {
        ...
        [WebMethod(EnableSession = true)]
        [ScriptMethod(ResponseFormat = ResponseFormat.Json, UseHttpGet = true)]
        public static object MyEncounterLoad()
        {
            if (ClientSession.UserName == string.Empty)
            {
                HttpContext.Current.Response.StatusCode = 999;
        ...
        [WebMethod(EnableSession = true)] 
        [ScriptMethod(ResponseFormat = ResponseFormat.Json, UseHttpGet = true)]
        public static object LoadMyTaskCompleted()
        {
            if (ClientSession.UserName == string.Empty)
            {
                HttpContext.Current.Response.StatusCode = 999;
        ...
        [WebMethod(EnableSession = true)]
        [ScriptMethod(ResponseFormat = ResponseFormat.Json, UseHttpGet = true)]
        public static object LoadMyTask()
        {
            if (ClientSession.UserName == string.Empty)
            {
                HttpContext.Current.Response.StatusCode = 999;
            ...
            var resultNew = new
            {
                data = Compress(JsonConvert.SerializeObject(MyTask.ToList<MyQ>())),
            };
            return resultNew;
        }
        [WebMethod(EnableSession = true)]
        ...
        [WebMethod(EnableSession = true)]
        [ScriptMethod(ResponseFormat = ResponseFormat.Json, UseHttpGet = true)]
        public static object LoadMyAmendment()
        {
            if (ClientSession.UserName == string.Empty)
            {
                HttpContext.Current.Response.StatusCode = 999;
        ...
        public static object chkShowAllMyEncounter()
        {
            ...
            string sShowall = HttpContext.Current.Request.Params["extra_search"];
            bool bValue = false;
            if (ConfigurationSettings.AppSettings["IsShowAllMyEncountersQueue"] == "Y")
            {
                bValue = true;
            }
            else
        ...
        public static bool ProcessEncounter(string[] data)
        {
            ...
            if (ClientSession.UserCurrentProcess == "MA_PROCESS" || ClientSession.UserCurrentProcess == "TECHNICIAN_PROCESS" || ClientSession.UserCurrentProcess == "PROVIDER_PROCESS" || ClientSession.UserCurrentProcess == "AKIDO_SCRIBE_PROCESS" || ClientSession.UserCurrentProcess == "SCRIBE_PROCESS" || ClientSession.UserCurrentProcess == "TRANSCRIPT_PROCESS") //to update DOS when Technician/MA/Provider processes the encounter for the first time -- CMG Ancilliary 
            {
        ...
        [WebMethod(EnableSession = true)]
        [ScriptMethod(ResponseFormat = ResponseFormat.Json, UseHttpGet = true)]
        public static object LoadEncounter()
        {
            if (ClientSession.UserName == string.Empty)
            {
                HttpContext.Current.Response.StatusCode = 999;
            ...
            bool bValue = false;
            if (ConfigurationSettings.AppSettings["IsShowAllGeneralEncountersQueue"] == "Y")
            {
                bValue = true;
            }
            else
            ...
            if (sViewAllFacilities == "Checked")
            {
                LoadMyQ = wfMngr.LoadMyQHashTable("ViewAllFacilities~" + ClientSession.FacilityName, ObjType, ProcessType, ClientSession.UserName, bValue, iDefaultDays, "", ShowAllObjType);// ClientSession.DefaultNoofDays);
            }
            else
            {
                LoadMyQ = wfMngr.LoadMyQHashTable(ClientSession.FacilityName, ObjType, ProcessType, ClientSession.UserName, bValue, iDefaultDays, "", ShowAllObjType);// ClientSession.DefaultNoofDays);
            }
        ...
        [WebMethod(EnableSession = true)]
        [ScriptMethod(ResponseFormat = ResponseFormat.Json, UseHttpGet = true)]
        public static object LoadEncounterTabClick()
        {
            if (ClientSession.UserName == string.Empty)
            {
                HttpContext.Current.Response.StatusCode = 999;
            ...
            var resultNew = new
            {
                data = Compress(JsonConvert.SerializeObject(MyQ)),
                role = ClientSession.UserRole
            };
            return resultNew;
        }
